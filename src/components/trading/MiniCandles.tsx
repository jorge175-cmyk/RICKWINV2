...
export interface MiniCandlesProps {
  ...
  invertPrice?: boolean;
  /** Destaca a vela de continuação, se enviada. */
...
export function MiniCandles({
  ...
  const series = nextCandle ? [...ordered, nextCandle] : ordered;
  ...
  return (
    ...
    >
      {series.map((candle, index) => {
        const isNext = nextCandle != null && index === series.length - 1;
      ...
      {nextCandle && (
        <line
          x1={(series.length - 1) * step}
          x2={(series.length - 1) * step}
          y1={0}
          y2={height}
          stroke="currentColor"
          strokeDasharray="3 3"
